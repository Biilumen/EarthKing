...
public class OffScreenIndicator : MonoBehaviour
{
    ...
    void DrawIndicators()
    {
        foreach(Target target in targets)
        {
            ...
            if (target.Character.Name != "Player")
            {
                isTargetVisible = OffScreenIndicatorCore.IsTargetVisible(screenPosition, target.transform);
            }
            ...
            if(indicator)
            {
                indicator.SetLable(target.Character,target.TargetColor); //Set the lable text for the indicator.
                indicator.SetImageColor(target.TargetColor);// Sets the image color of the indicator.
                indicator.SetDistanceText(target.GetScore()); //Set the distance text for the indicator.
