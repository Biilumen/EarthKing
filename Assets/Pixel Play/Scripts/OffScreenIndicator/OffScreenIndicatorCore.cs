...
namespace PixelPlay.OffScreenIndicator
{
    public class OffScreenIndicatorCore
    {
        ...
        /// <returns></returns>
        public static bool IsTargetVisible(Vector3 screenPosition, Transform target)
        {
            RaycastHit hit;
            Ray ray = new Ray(Camera.main.transform.position, target.position - Camera.main.transform.position);
            Physics.Raycast(ray, out hit);
            bool isTargetVisible = screenPosition.z > 0 && screenPosition.x > 0 && screenPosition.x < Screen.width && screenPosition.y > 0 && screenPosition.y < Screen.height && !hit.collider.TryGetComponent(out Barier barier);
